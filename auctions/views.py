...
def index(request):
    ...
    itemss=Listing.objects.raw("SELECT * FROM auctions_listing GROUP BY category")
    try:
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            return render(request, "auctions/login.html", {
                "message": messages.success(request,"Invalid username and/or password.", "danger")
            })
    else:
        return render(request, "auctions/login.html")
    ...
    if request.method == "POST":
        ...
        if password != confirmation:
            return render(request, "auctions/register.html", {
                "message":  messages.success(request,"Passwords Must Match.", "warning")
            })
        ...
        try:
            ...
            return render(request, "auctions/register.html", {
                "message":  messages.success(request,"Username Is Already Taken", "warning")
            })
        login(request, user)
        return HttpResponseRedirect(reverse("index"))
    else:
...
def delete(request , listingid):
    Listing.objects.get(id=listingid).delete()
    messages.success(request,"Auction Deleted Successfully", "success")
    return redirect('index')